...
class PBS_NLHE:
    ...
    def create_infostate_to_impossible_infostate_matrix(self) -> torch.Tensor:
        infostate_to_possible_infostates = torch.ones(self.NLHE_games.amount_tables, self.amount_infostates, self.amount_infostates)
    ...
    def get_reward(self, actions: torch.Tensor, idx_of_player_to_act: list[int]) -> torch.Tensor:
        ...
        if len(idx_of_showdown_tables) > 0:
            equity_per_infostate_per_player = self.get_equity_of_players(idx_of_showdown_tables)     
            for i, table_idx in enumerate(idx_of_showdown_tables):
                print(equity_per_infostate_per_player[i])
                print(self.public_belief_state)
                rewards[table_idx] += equity_per_infostate_per_player[i] 
        return rewards
    
    def get_showdown_ranking(self, idx_of_showdown_tables: list[int]) -> torch.Tensor:
        ...
        for i, table_idx in enumerate(idx_of_showdown_tables):
            for j, hand in enumerate(self.all_hands):
                ...
                if self.public_belief_state[table_idx, j].sum() == 0:
                    continue
                try:
                    ranking[i, j] = evaluate_cards(*[card.to_string() for card in self.NLHE_games.tables[table_idx].community_cards], *hand)
                except:
                    print("public belief state", self.public_belief_state[table_idx])
                    raise ValueError(f"Error in evaluate_cards with community cards {[card.to_string() for card in self.NLHE_games.tables[table_idx].community_cards]} and hand {hand}")
    ...
    def get_equity_of_players(self, idx_of_showdown_tables: list[int]) -> torch.Tensor:
        ranking = self.get_showdown_ranking(idx_of_showdown_tables)
        ...
        for player in range(self.amount_players):
            ...
            infostate_matrix = self.public_belief_state[:,:,player:player + 1].transpose(2, 1) * self.infostate_matrix
